...
class TokenInfo(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        with open("verified_tokens.json") as tokens:
            ...
            for info in data:
                for ticker, information in info.items():
                    self.token_dict[ticker] = information
    ...
    async def price_of(self, interaction: discord.Interaction, ticker: str):
        ...
        volume_url = (
            f"https://www.taptools.io/_next/data/0AuaWVMAmZCqDgtUBTP1r/charts/token.json?pairID"
            f"=0be55d262b29f564998ff81efe21bdc0022621c12f15af08d0f2ddb1.{pool_id}&currency=ADA"
        )
        ...
        if token_data_status == 200:
            ...
            price_embed.add_field(name="24h Volume", value=f"{daily_volume :,.02f} ₳")
            price_embed.add_field(
                name="Supply", value=f"{circulating:,.0f}", inline=False
            )
            price_embed.add_field(name="Holders", value=f"{holders:,.0f}", inline=True)
            price_embed.add_field(
                name="Diluted M.Cap",
                value=f"{marketcap:,.0f} ₳",
            )
